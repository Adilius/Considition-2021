...
class rl_solve:
    ...
    def __init__(self, game_info):
        ...
        for package in self.packages:
            self.amountOfPackages = self.amountOfPackages + 1
            self.weightPackages.append({"weight": package["weightClass"], "id": package["id"]})
            self.orderPackages.append({"order": package["orderClass"], "id": package["id"]})
            self.areaPackages.append({"area": package["width"]*package["length"], "id": package["id"]})
            self.volumePackages.append({"volume": package["width"]*package["length"]*package["height"], "id": package["id"]})
        ...
        self.heavyPackages = sorted(self.heavyPackages, key=lambda i: (i['area']))
        self.otherPackages = sorted(self.otherPackages, key=lambda i: (i['area']))

        self.weightPackages = sorted(self.weightPackages, key=lambda i: (i['weight']))
        self.orderPackages = sorted(self.orderPackages, key=lambda i: (i['order']))
        self.areaPackages = sorted(self.areaPackages, key=lambda i: (i['area']))
        self.volumePackages = sorted(self.volumePackages, key=lambda i: (i['volume']))
    ...
    def AddPackage(self, package):
        self.placedPackages.append(
            {
            "id": package["id"], "x1": self.xp, "x2": self.xp, "x3": self.xp, "x4": self.xp,
